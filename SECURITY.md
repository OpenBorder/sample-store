...
- The hosted server starts with transaction routes disabled, refuses live keys when enabled, and
  accepts only the exact production-dashboard Sandbox API host.
...
- Terminal order state changes require a timestamped, authentic raw-body webhook; duplicate
  deliveries are durably ignored.
